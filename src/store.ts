...
interface GameState {
  ...
  focusedRoom: FocusedRoom | null;
  selectedInteraction: SceneInteraction | null;

  // Actions
  setCameraMode: (mode: CameraMode) => void;
  focusRoom: (room: FocusedRoom) => void;
  clearFocusedRoom: () => void;
  selectInteraction: (interaction: SceneInteraction) => void;
...
const INITIAL_QUESTS: Quest[] = [
  ...
  },
  {
    ...
    description: "Try making pasta from scratch",
    category: "cooking",
    expReward: 60,
    completed: false,
    type: "daily",
  ...
  },
  {
    ...
    description: "Visit a new local park",
    category: "travel",
    expReward: 100,
    completed: false,
    type: "side",
  ...
  },
  {
    ...
    description: "Eat only plant-based food",
    category: "cooking",
    expReward: 120,
    completed: false,
    type: "experiment",
...
const POSSIBLE_COLLECTIONS: Collection[] = [
  {
    ...
    description: "Awarded for exceptional fitness.",
    icon: "🏋️",
    category: "fitness",
  },
  {
    id: "c2",
    name: "Ancient Tome",
    description: "A rare book of wisdom.",
    icon: "📖",
    category: "reading",
  },
  {
    id: "c3",
    name: "Chef's Star",
    description: "Mastery of the culinary arts.",
    icon: "⭐",
    category: "cooking",
  },
  {
    id: "c4",
    name: "Zen Crystal",
    description: "Pure peace of mind.",
    icon: "🔮",
    category: "mentalHealth",
  },
  {
...
const INITIAL_SKILLS: Skill[] = [
  ...
  {
    id: "s11",
    name: "Practical Skills (Finance, First Aid)",
    level: 1,
    exp: 0,
    maxExp: 100,
    category: "practical",
...
export const useGameStore = create<GameState>()(
  persist(
    (set) => ({
      ...
      clearFocusedRoom: () => set({ focusedRoom: null }),
      selectInteraction: (interaction) =>
        set({ selectedInteraction: interaction }),
      clearInteraction: () => set({ selectedInteraction: null }),
      ...
      completeQuest: (id) =>
        set((state) => {
          ...
          if (Math.random() < 0.1) {
            const unowned = POSSIBLE_COLLECTIONS.filter(
              (pc) => !state.collections.find((c) => c.id === pc.id),
            );
            if (unowned.length > 0) {
          ...
          const relatedSkills = state.skills.filter(
            (s) =>
              s.category === quest.category ||
              quest.category === "mentalHealth",
          );
          let newSkills = [...state.skills];
          if (relatedSkills.length > 0) {
            const targetSkill =
              relatedSkills[Math.floor(Math.random() * relatedSkills.length)];
          ...
          return {
            ...
            stats: {
              ...state.stats,
              [quest.category]: state.stats[quest.category]
                ? state.stats[quest.category] + 1
                : 1,
            },
          };
        }),
      ...
      completeOnboarding: (skills, vices, subGoals) =>
        set((state) => ({
          isOnboarded: true,
          skills: skills,
          subGoals: subGoals,
